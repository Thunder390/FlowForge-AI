 ...
 *
 * ## Why this is in `registry` rather than in `ai`
 *
 * It shipped in `ai` in M5, because `ai` was the only package that both held a
 * registry and knew FFIR. M6 added a second: the compiler's stage 1 has to run
 * the full gate, since the compiler is a public library boundary and must not
 * assume its caller validated. But `ai` and `compiler` are siblings and neither
 * may import the other, so a walk reachable from only one of them cannot be the
 * whole gate for both. Duplicating it would put the same eighteen rules behind
 * two implementations that drift.
 *
 * It belongs here on the merits, not just by elimination. The rules it applies
 * are registry data, `validate-params.ts` already holds them, and the only two
 * things this module needs are FFIR types and a `Registry`. `ai` re-exports all
 * three functions, so nothing about its public surface changed.
 *
 * ## What this module deliberately does not do
 *
 * It never reads a binding. A validator knowing that `slack.message.send` is
...
import { capabilitiesOfIntegration, resolve } from "./resolve.js";
import type { Registry } from "./types.js";
import { validateParameters } from "./validate-params.js";
...
export function checkRegistry(doc: FFIRDocument, registry: Registry): ValidationResult {
  ...
  doc.nodes.forEach((node, index) => {
    ...
    const integration = integrationSegment(node.capability);
...
/**
 * Runs both stages this module owns.
 *
