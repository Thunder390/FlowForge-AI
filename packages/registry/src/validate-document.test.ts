...
import type { Capability, Registry } from "./types.js";
import {
  checkParameters,
  checkRegistry,
...
describe("architectural compliance", () => {
  it("validates identically against a registry with every binding removed", () => {
    // These stages never read a binding, whichever package they live in. If
    // they did, "adding Make.com requires no AI change" would stop being true,
    // because `ai` calls exactly these functions. A validator is the easiest
    // place for that leak to start: "check it can compile" sounds like
    // diligence. Whether a target can express a capability is settled at
    // registry load and at the compile dry-run, which is stage 5 and belongs to
    // the pipeline.
